...
void open_door(WebsocketsClient &client) {
  if (digitalRead(relay_pin) == HIGH) {
    ...
    digitalWrite(Red,LOW); //close (energise) relay so door unlocks
...
void loop() {
  ...
  while (client.available()) {
    ...
    if (g_state == START_DETECT || g_state == START_ENROLL || g_state == START_RECOGNITION)
    {
      ...
      else if (g_state == START_RECOGNITION) {

    static unsigned long last_no_face = 0;
    if (millis() - last_no_face > 2000) {
